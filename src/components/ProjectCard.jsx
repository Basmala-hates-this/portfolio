import ScreenshotSlider from './ScreenshotSlider'

export default function ProjectCard({ name, blurb, tech, status, repo, liveLink, image, images, features }) {
  return (
    <div className="flex flex-col sm:flex-row gap-6 border-3 border-success  rounded-lg p-5">
      <div className="sm:w-2/5 shrink-0">
        {images && images.length > 0 ? (
          <ScreenshotSlider images={images} />
        ) : image ? (
          <img
            src={image}
            alt={name}
            className="w-full rounded-md object-cover"
          />
        ) : null}
      </div>

      <div className="flex-1">
        <h4 className="text-lg font-semibold text-text-soft">{name}</h4>
        <p className="text-text-muted mt-1">{blurb}</p>

        {features && features.length > 0 && (
          <ul className="list-disc list-inside text-sm text-text-muted mt-2 space-y-1">
            {features.map((f) => (
              <li key={f}>{f}</li>
            ))}
          </ul>
        )}

        <p className="text-sm text-text-muted mt-2">{tech}</p>
        {status && <p className="text-sm text-text-muted italic mt-1">{status}</p>}
        <div className="flex gap-3 mt-2">
          {repo && <a href={repo} 
          className="no-underline text-success
        hover:text-text-main
  transition-all px-6 py-4 rounded-md 
   translate-y-0 hover:translate-y-1
   hover:shadow-[0_8px_20px_var(--color-accent-neon)]
   focus:outline-none focus:ring-2 focus:ring-text-main hover:bg-bg-dark2
"
          >GitHub</a>}
          {liveLink && <a href={liveLink} className="no-underline text-success
        hover:text-text-main
  transition-all px-6 py-4 rounded-md 
   translate-y-0 hover:translate-y-1
   hover:shadow-[0_8px_20px_var(--color-accent-neon)]
   focus:outline-none focus:ring-2 focus:ring-text-main hover:bg-bg-dark2
">Live</a>}
        </div>
      </div>
    </div>
  )
}